...
public class MisionerosCanibalesDemo {
    ...
    private static void mcDepthLimitedGraphDemo() {
        ...
        try {
            Problem problem = new Problem(boardInicial, FarolilloFunctionFactory.getActionsFunction(), FarolilloFunctionFactory.getResultFunction(), new FarolilloGoalTest());
            int limitDepth = 6;
    ...
    private static void farolilloAStarDemo() {
        System.out.println("\nfarolilloDemo AStar Search (FarolilloHeuristic)-->");
        try {
            Problem problem = new Problem(boardInicial, FarolilloFunctionFactory.getActionsFunction(), FarolilloFunctionFactory.getResultFunction(), new FarolilloGoalTest());
            SearchForActions search = new AStarSearch(new GraphSearch(), new FarolilloHeuristicFunction());
            SearchAgent agent = new SearchAgent(problem, search);
            printActions(agent.getActions());
            printInstrumentation(agent.getInstrumentation());
        } catch (Exception e) {
